...
class Explainer(object):
    ...
    def parse_plaintext_explanation(self, content):
        ...
        while len(lines) > 0:
            ...
            indexed_comments = [(start, length, comment) for
                    ((start, length), comment) in indexed_comments]
    ...
    def annotate_indexed_comments(self, cmd, indexed_comments):
        ...
        for (start, length, _) in indexed_comments:
            ...
            drawing[0][start + int(length / 2)] = self.symbols.joint
...
if __name__ == '__main__':
    ...
    try:
        ...
        print('Symbol preset %r unsupported.' % options.preset.upper(),
              file=sys.stderr)
        sys.exit(1)
    ...
    if len(args) > 0:
        ...
        for i in args:
            try:
                ...
                print('Can\'t read %s: %s' % (i, e), file=sys.stderr)
                sys.exit(1)
    else:
        try:
